"""
Create by DevidCopperfild. 
thanks for supporting PancakeCompany.


ENG:
Assistant can:
* recognize and synthesize speech in offline mode (without access to the Internet);
* report the weather forecast anywhere in the world;
* make a search query in the Google search engine
  (as well as open the list of results and the results of this query themselves);
* make a search query for a video in the YouTube system and open a list of the results of this query;
* search for a definition in Wikipedia with further reading of the first two sentences;
...
* play a random goodbye with the subsequent completion of the program;
* change the settings of the language of recognition and speech synthesis;

Note for DevOps:
The voice assistant uses the capabilities built into the Windows 10 operating system for speech synthesis
(i.e. voices are operating system dependent). The pyttsx3 library is used for this.

For the correct operation of the speech recognition system in combination with the SpeechRecognition library
using the PyAudio library to get sound from the microphone.

To install PyAudio, you can find and download the required whl file, depending on the architecture and version of Python, here:
https://www.lfd.uci.edu/~gohlke/pythonlibs/#pyaudio

After downloading the file to the project folder, the installation can be launched using a similar command:
...
vosk, whl file for which can be found here depending on the required architecture and version of Python:
https://github.com/alphacep/vosk-api/releases/

After downloading the file to the project folder, the installation can be launched using a similar command:
pip install wax-0.3.7-cp38-cp38-win_amd64.whl

To get weather forecast data, I used the OpenWeatherMap service, which requires an API key.
You can get an API key and read the documentation after registration (there is a Free plan) here:
https://openweathermap.org/

Commands for installing other third-party libraries:
pip install google
